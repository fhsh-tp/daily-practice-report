...
- [x] 6.1 撰寫 `tests/auth/test_permissions.py`：覆蓋 flag 組合（Permission flag check with bitwise AND）、各 preset 組成驗證（Student preset grants self and read permissions 等）
- [x] 6.2 撰寫 `tests/auth/test_guards.py`：覆蓋 guard 放行（Authorized request passes guard）與 403 拒絕（Unauthorized request blocked by guard）
